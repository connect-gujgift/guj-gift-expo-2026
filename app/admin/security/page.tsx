...
export default function SecurityDeptPage() {
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    // 1. Check if the input is a UUID (from QR Code) or a Phone Number
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(scanInput)

    if (isUUID) {
      // Search Visitors by ID
      const { data: vData } = await supabase.from('visitors').select('*').eq('id', scanInput).maybeSingle()
      if (vData) { foundUser = vData; userType = 'VISITOR' }
      
      // Search Exhibitors/Staff by ID if not found
      if (!foundUser) {
        const { data: eData } = await supabase.from('exhibitors').select('*').eq('id', scanInput).maybeSingle()
        if (eData) { foundUser = eData; userType = eData.is_staff ? 'STAFF' : 'EXHIBITOR' }
      }
    } else {
      // Search by Phone Number
      const { data: vData } = await supabase.from('visitors').select('*').eq('phone', scanInput).maybeSingle()
      if (vData) { foundUser = vData; userType = 'VISITOR' }
      
      if (!foundUser) {
        const { data: eData } = await supabase.from('exhibitors').select('*').eq('phone', scanInput).maybeSingle()
        if (eData) { foundUser = eData; userType = eData.is_staff ? 'STAFF' : 'EXHIBITOR' }
      }
    }
    ...
    if (foundUser) {
      ...
      // Add to local history log (keep last 10)
  ...
  // Quick Action: Grant VIP Status on the spot
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-7 space-y-6">
              ...
              <CardContent className="p-8">
                <form onSubmit={handleScan} className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    />
                    <Button type="submit" disabled={isScanning} className="h-16 px-8 bg-red-600 hover:bg-red-700 font-black uppercase tracking-widest rounded-2xl text-white shadow-lg">
                      {isScanning ? '...' : 'Scan'}
                    </Button>
                  </div>
                </form>
            ...
            {scanStatus === 'IDLE' && (
              <div className="h-[400px] border-4 border-dashed border-slate-700 rounded-[2rem] flex flex-col items-center justify-center text-slate-600">
          ...
          <div className="md:col-span-5">
            <Card className="border-0 shadow-2xl rounded-[2rem] bg-slate-800 h-full flex flex-col overflow-hidden">
