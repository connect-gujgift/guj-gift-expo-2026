      ...
      <section className="relative h-[700px] flex items-center justify-center text-center text-white bg-slate-900 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl px-6 space-y-8 animate-in fade-in zoom-in duration-700">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center pt-6">
            ...
            <Link href="/login">
              <Button size="lg" variant="outline" className="w-full sm:w-auto text-sm px-8 py-8 bg-white/5 hover:bg-white/10 text-white border-white/20 backdrop-blur-md font-black uppercase tracking-widest rounded-2xl transition-all active:scale-95">
                Exhibitor Login
              </Button>
            </Link>
...
function FeatureCard({ icon, title, desc }: { icon: string, title: string, desc: string }) {
  return (
    <div className="group p-8 bg-slate-50 rounded-[2.5rem] border-b-4 border-transparent hover:border-orange-500 hover:bg-white hover:shadow-2xl transition-all duration-300 cursor-pointer">
