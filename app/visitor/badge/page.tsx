...
function VisitorBadgeContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Fetch specifically from visitors table
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-10 px-4 print:p-0 print:bg-white font-sans">
      ...
      <Button variant="ghost" onClick={() => router.push('/visitor')} className="self-start md:self-auto md:absolute md:left-4 md:top-4 mb-4 text-slate-400 font-bold uppercase text-[10px] tracking-widest print:hidden">
        ← Back to Hub
      </Button>

      {/* 4x6 INCH STANDARD CONTAINER */}
      <div 
        ref={badgeRef}
